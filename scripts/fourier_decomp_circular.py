...
def build_complex_basis(i0, j0, m0, n0, aperture_bools, n_ap=1788):
    ...
    A_filter = np.zeros((n_ap, i0*j0), dtype=complex)
    c = 0
...
def decompose_images(i0, j0, m0, n0, resolution, t_frames, data, A,
        aperture_bools, save, rcond=1e-6):
    ...
    for i in range(t_frames):
        print(i)
        img = apply_binning(data[:,:, i], resolution, i0)[aperture_bools]
...
#data_file = '../data/turbulence_v=14_franken_AOres=48_2khz_infinite_500nm_90000.fits'
#data_file = '/data/users/jumfowle/outputs/turbulence_poyneer_8_franken_AOres=144_2khz_lessmodes.fits'
data_file = '../data/turbulence_chun_franken_AOres=48_2khz_infinite_500nm_240000.fits'
print(f'Reading data from {data_file}')
training = convert_phase_to_wfe(fits.getdata(data_file, 0)) #[8:40, 8:40, :]
future = convert_phase_to_wfe(fits.getdata(data_file, 1)) #[8:40, 8:40, :]
...
_, A = build_complex_basis(i0, j0, m0, n0, aperture_bools)
modes = decompose_images(i0, j0, m0, n0, resolution, t_frames, training, A,
        aperture_bools,
        save='modes=48_frames=90000_8_ptt_chun_direction_2khz_circuluar_training.fits')
modes = decompose_images(i0, j0, m0, n0, resolution, 10000, future, A, aperture_bools,
        save='modes=48_frames=10000_8_ptt_chun_direction_2khz_circular_future.fits')
