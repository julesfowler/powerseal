...
def build_atmosphere(wavelength, pupil_grid, model='single', remove_modes=True):
    ...
    def build_multilayer_model(input_grid):
        ...
        for h, v, cn, L0 in zip(heights, velocities, cn_squared,outer_scales):
            layers.append(FiniteAtmosphericLayer(input_grid, cn, L0, v, h))
        return layers 

    if model == 'single':
        print('Building single layer.')
        cn_squared = Cn_squared_from_fried_parameter(0.20, wavelength=wavelength)
        layer = FiniteAtmosphericLayer(pupil_grid, cn_squared, 20, 7)
...
past, future = build_sample_data(i, j, k, n_iters, resolution=resolution, data_type='AO', save='/data/users/jumfowle/outputs/turbulence_poyneer_8_franken_AOres=144_2khz_finite.fits')
