    ...
    if data_type == 'AO':
        ...
        for iters in [(k, past), (n_iters, future)]:
            for index in range(iters[0]):
                ...
                coeffs[:32*32] = 0.0
                phase -= high_energy_modes.linear_combination(coeffs)
                phase = phase.shaped
...
past, future = build_sample_data(48, 48, k, n_iters, resolution=48, data_type='AO', save='/data/users/jumfowle/outputs/turbulence_1_8_franken_AOres=48_2khz_infinite_500nm_60000.fits')
