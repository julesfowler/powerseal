...
def build_complex_basis(i0, j0, m0, n0):
    ...
    for i in range(int(-1*i0/2+1), int(i0/2+1)):
        for j in range(0, int(j0/2+1)):
            for m in range(m0):
                for n in range(n0):
                    print(m, n, c) 
                    i_imag = complex(0, 1) 
                    A[m, n, c] = complex(np.cos(i_imag*(i*m + j*n)*2*np.pi/m0), np.sin(i_imag*(i*m + j*n)*2*np.pi/m0))
            c+=1
...
data_file = '/data/users/jumfowle/outputs/turbulence_poyneer_8_franken_AOres=144_2khz.fits'
...
modes = decompose_images(i0, j0, m0, n0, resolution, t_frames, data, A, save='modes=48_frames=8192_poyneer_8_franken_2khz_complex_conjugate.fits')
