...
def build_atmosphere(wavelength, pupil_grid, model='single', remove_modes=True):
    ...
    high_energy_modes = None 
    if remove_modes:
        print('Building HE fourier modes.')
        # Make high energy fourier modes
        fourier_grid = make_fft_grid(pupil_grid, 1, 1)
        high_energy_modes = make_fourier_basis(pupil_grid, fourier_grid, sort_by_energy=True)
	
        # Scoop only high energy ones
        #high_energy_modes.transformation_matrix = high_energy_modes.transformation_matrix[:, 1152:]
    ...
    return layer , high_energy_modes
...
def build_sample_data(i, j, k, n_iters, data_type='sin', save=None, resolution=None):
    ...
    past_ptt = np.ones((i, j, k))
    past_franken = np.ones((i, j, k))
    future_ptt = np.ones((i, j, n_iters))
    future_franken = np.ones((i, j, n_iters))
    ...
    if data_type == 'AO':
        ...
        for iters in [(k, past_ptt, past_franken), (n_iters, future_ptt, future_franken)]:
            for index in range(iters[0]):
                layer.t = 1/(2e3)*(index+1)
                print(index)
                phase = layer.phase_for(science_wavelength)
                coeffs = high_energy_modes.coefficients_for(phase)
                coeffs[:32*32] = 0.0
                phase_franken = phase - high_energy_modes.linear_combination(coeffs)
                phase = phase.shaped
                phase_franken = phase_franken.shaped
                iters[1][:, :, index] = apply_binning(phase, resolution, i)
                iters[2][:, :, index] = apply_binning(phase_franken, resolution, i)
...
#save = 'prediction_results_telemetry.fits'
