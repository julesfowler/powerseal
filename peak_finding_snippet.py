...
fs = 2e3
window = signal.get_window(window='hamming', Nx=2000)
...
mode_data = ascii.read('data/test_mode_index.csv')

mode_psds = {}
mode_layers = {}
for i, mode_index in enumerate(mode_data['mode']):
    print(mode_index)
    ...
    print('Sigmas: ', sigmas)
    alpha_length = np.zeros(100)
    alpha_length[:len(alphas)] = alphas
    sigma_length = np.zeros(100)
    sigma_length[:len(sigmas)] = sigmas
    #plt.show()
    mode_layers[f'{str(mode_index)}_alphas'] = alpha_length
...
#print(mode_layers)
#print(mode_psds)
ascii.write(mode_layers, 'mode_layers.csv', overwrite=True)
ascii.write(mode_psds, 'mode_psds.csv', overwrite=True)
